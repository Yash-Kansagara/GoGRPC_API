...
func (s *Server) AddExecs(ctx context.Context, req *pb.Execs) (*pb.Execs, error) {
	...
	// insert into mongodb
	res, err := mongodb.ExecsCollection.InsertMany(ctx, newTBson)
	if err != nil {
...
func (s *Server) DeleteExecs(ctx context.Context, req *pb.ExecIds) (*pb.DeleteExecsResponse, error) {
	...
	// delete from mongodb
	res, err := mongodb.ExecsCollection.DeleteMany(ctx, bson.M{"_id": bson.M{"$in": objectIds}})
	if err != nil {
...
func (s *Server) GetExecs(ctx context.Context, req *pb.GetExecsReq) (*pb.Execs, error) {
	...
	cursor, err := mongodb.ExecsCollection.Find(ctx, filter, opt)
	if err != nil {
		return &pb.Execs{
			Execs: nil,
...
func (s *Server) UpdateExecs(ctx context.Context, req *pb.Execs) (*pb.Execs, error) {
	...
	for _, exec := range req.GetExecs() {
		...
		temp.Id = ""
		res, err := mongodb.ExecsCollection.UpdateOne(ctx, bson.M{"_id": id}, bson.M{"$set": temp})
		if err != nil {
