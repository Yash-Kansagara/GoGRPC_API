...
func AddTeachersToDB(ctx context.Context, req *pb.Teachers) (*pb.Teachers, error) {
	...
	// insert into mongodb
	res, err := TeachersCollection.InsertMany(ctx, newTBson)
	if err != nil {
...
func DeleteTeachersFromDB(ctx context.Context, req *pb.TeacherIds) (*pb.DeleteTeacherResponse, error) {
	...
	// delete from mongodb
	res, err := TeachersCollection.DeleteMany(ctx, bson.M{"_id": bson.M{"$in": objectIds}})
	if err != nil {
...
func GetTeachersFromDB(ctx context.Context, req *pb.GetTeachersReq) (*pb.Teachers, error) {
	...
	cursor, err := TeachersCollection.Find(ctx, filter, opt)
	if err != nil {
		return &pb.Teachers{
			Teachers: nil,
...
func UpdateTeachersInDB(ctx context.Context, req *pb.Teachers) (*pb.Teachers, error) {
	...
	for _, teacher := range req.GetTeachers() {
		...
		temp.Id = ""
		res, err := TeachersCollection.UpdateOne(ctx, bson.M{"_id": id}, bson.M{"$set": temp})
		if err != nil {
...
func GetStudentsByClassTeacherFromDB(ctx context.Context, req *pb.TeacherId) (*pb.Students, error) {
	...
	cursor, err := StudentsCollection.Find(ctx, studentsfilter)
	if err != nil {
		return &pb.Students{
			Students: nil,
...
func GetStudentCountByTeacherFromDB(ctx context.Context, req *pb.TeacherId) (*pb.StudentCount, error) {
	...
	count, err := StudentsCollection.CountDocuments(ctx, filter)
	if err != nil {
...
func GetTeacherById(ctx context.Context, teacherId string) (*models.Teacher, error) {
	...
	res := TeachersCollection.FindOne(ctx, filter)
